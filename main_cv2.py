...
class OpenCVDetector:
    def detect(self, img):# [[[type: LineType, start, end], line, ..., line], obstacls]
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        template = cv2.imread('obstacle_template.jpg', 0)
        th, tw = template.shape

        locs = []

        threshold = 0.4
        for scale in np.linspace(0.4, 1, num=10):
            #print(scale)
            s_tw, s_th = (int(tw * scale), int(th * scale))
            templ_resized = cv2.resize(template, (s_tw, s_th))
            ...
            for pt in zip(*loc[::-1]):
                x, y = pt
                locs.append([pt, (s_tw, s_th)])
                cv2.rectangle(img_copy, pt, (x + s_tw, y + s_th), (0,0,255), 2)
        
        if len(locs) > 0:# obstacle detected
            print(locs)
            rects = np.array(locs).reshape((-1, 4))
            print(rects)
            average = np.average(rects, axis=0)
            average = map(int, map(round, average))
            print(average)
            x, y, ow, oh = average
            cv2.rectangle(img, (x, y), (x + ow, y + oh), (0,0,255), 2)
          
        cv2.imshow('CV detection', img)
        cv2.waitKey(1)
'''
        sigma = 0.33
        v = np.median(gray)
...
def test_with_image():
    ...
    dwa = DWA()
    obj_det = OpenCVDetector()
    path = os.path.join('api', 'projective_transformation', 'images', 'img4.jpg')
    img = cv2.imread(path)
    do_for_frame(img, obj_det, dwa, servo)
